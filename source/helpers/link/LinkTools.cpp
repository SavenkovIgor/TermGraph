...
public:
    ...
    static bool linkAndTermSimilarWordDistance(const QString& link, const QString& term)
    {
        ...
        auto stringSizeDiffer = abs(term.size() - link.size());
    ...
    static int levDistance(QStringView src, QStringView dst, int limit = 100000)
    {
        ...
        for (ulong i = 1; i <= m; i++) {
            min_in_row = numeric_limits<int>::max();
            for (ulong j = 1; j <= n; j++) {
    ...
    static optional<int> linkAndTermDistance(const QString& link, const QString& term, int maxLimit)
    {
        ...
        int wordsCountInLink = TextTools::wordCount(link);
        int maxWordDistance  = min(4 * wordsCountInLink, maxLimit);
